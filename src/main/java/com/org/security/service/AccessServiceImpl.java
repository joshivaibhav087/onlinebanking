	...
	public String resetPasswordById(String id,PasswordResetRequest passwordResetRequest) throws Exception {
		...
		if(mydata.isPresent()) {
			...
			if(encoder.matches(passwordResetRequest.getOldPassword(), mydata.get().getPassword())) {
				User user = new User(id,mydata.get().getName(),mydata.get().getEmailId(),mydata.get().getMobile(),mydata.get().getGender(),mydata.get().getAdhar_number(),mydata.get().getPan(),encoder.encode(passwordResetRequest.getNewPassword()));
